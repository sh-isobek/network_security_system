...
$cmd = @"
`$l='C:\ProgramData\NetworkSecurityAgent\gpp-task.log'; try { New-Item -ItemType Directory -Force (Split-Path `$l) | Out-Null } catch {}; function W(`$m) { try { Add-Content -Path `$l -Value ((Get-Date -Format s)+' '+`$m) } catch {} }; W 'vazifa boshlandi'; `$ok=`$null; foreach (`$c in '$deployPath','$dcShare') { `$j = Start-Job -ScriptBlock { param(`$p) Test-Path `$p } -ArgumentList `$c; if (Wait-Job `$j -Timeout 45) { `$r = Receive-Job `$j } else { `$r = 'TIMEOUT' }; Remove-Job `$j -Force -ErrorAction SilentlyContinue; W ('Test-Path '+`$c+' -> '+`$r); if (`$r -eq `$true) { `$ok=`$c; break } }; if (`$ok) { W 'Deploy ishga tushdi'; try { `$p = Start-Process powershell.exe -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-File',`$ok -RedirectStandardOutput 'C:\ProgramData\NetworkSecurityAgent\gpp-deploy.out' -RedirectStandardError 'C:\ProgramData\NetworkSecurityAgent\gpp-deploy.err' -WindowStyle Hidden -PassThru; if (`$p.WaitForExit(480000)) { W ('Deploy tugadi, exit='+`$p.ExitCode) } else { W ('Deploy 8 daqiqada tugamadi: pid='+`$p.Id+' cpu='+`$p.CPU); foreach (`$c in Get-CimInstance Win32_Process -Filter ('ParentProcessId='+`$p.Id)) { W ('  bola: '+`$c.Name+' pid='+`$c.ProcessId+' '+`$c.CommandLine) }; Stop-Process -Id `$p.Id -Force } } catch { W ('Deploy XATO: '+`$_) } } else { W 'Deploy skripti topilmadi/erishilmadi' }
"@
